import { 
        ...
        let statusAssinaturaAtual = ""; // espelho do statusAssinatura vindo do onSnapshot
        let fullEmpresaData = null;

        // Guarda autoritativo em JS. O "disabled" do botão é cosmético/bypassável;
        // esta checagem no topo de cada mutação de pontos é o bloqueio real.
        function bloquearSeInativa() {
            if (writesBlocked) {
                showToast("Assinatura inativa — regularize para continuar operando.", "error");
                return true;
            }
            return false;
        }
        ...
        function validateAjusteForm() {
            ...
            if (justificativaLength >= 5 && !writesBlocked) {
        ...
        // Validação CPF/CNPJ com dígitos verificadores (módulo 11).
        ...
        document.getElementById("btn-pix-submit").addEventListener("click", async () => {
            // Evita criar nova assinatura no Asaas quando já está ativa (ex.: reabrir o modal).
            if (statusAssinaturaAtual === "active") {
                showToast("Sua assinatura já está ativa.", "success");
                return;
            }
            ...
            try {
                ...
                if (err.code === "unavailable") {
                    showToast("A cobrança PIX ainda está sendo gerada. Tente novamente em instantes.", "warn");
                } else if (err.code === "unauthenticated" || err.code === "permission-denied") {
        ...
        function listenEmpresaStatus() {
            ...
            onSnapshot(empRef, (snap) => {
                ...
                statusAssinaturaAtual = status; // espelho para o checkout (evita duplicidade)
        ...
        function setWriteButtonsDisabled(disabled) {
            ...
            // id-based — apenas mutações de pontos. Salvar Link (slug) e Apagar conta
            // (LGPD) NÃO entram no paywall: gerir o link e exercer o direito de
            // exclusão devem funcionar sempre, independentemente do status.
            const idButtons = [
