import { 
        ...
        let visualConfig = {};
        // Latch da assinatura inativa: quando true, bloqueia ações de pontuação/resgate
        // e mostra o banner persistente. Definido proativamente no branding e reforçado
        // (defesa em profundidade) ao primeiro erro ASSINATURA_INATIVA do backend.
        let lojaInativa = false;
        ...
        async function fetchCompanyBranding() {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    // Detecção proativa de assinatura inativa — MESMA regra do backend:
                    // active = "active" OU ("trial" com trialEndDate futuro). Qualquer
                    // outro caso (overdue/canceled/trial expirado) => inativa.
                    const ativa = (data.statusAssinatura === "active") || emTrial;
                    if (!ativa) aplicarLojaInativa();
        ...
        // Aplica o estado de loja inativa: trava o latch, mostra o banner persistente
        // e desabilita TODAS as ações de pontuação/resgate. Usado na detecção proativa
        // (branding) e no latch por erro do backend. Seleciona por id (sem depender de
        // vars que podem não existir).
        function aplicarLojaInativa() {
            lojaInativa = true;
            const banner = document.getElementById("inativa-banner");
            if (banner) banner.classList.replace("hidden", "flex");
            ["btn-start-scanner", "btn-manual-submit", "btn-add-points", "btn-remove-point", "btn-deliver-prize"]
                .forEach((id) => { const b = document.getElementById(id); if (b) b.disabled = true; });
            // Encerra o leitor para a câmera não disparar leituras condenadas.
            if (typeof teardownScanner === "function") teardownScanner().catch(() => {});
        }
        ...
        async function addPoints() {
            ...
            try {
                ...
                // Não reabilita se a loja latchou inativa (senão volta a martelar).
                if (btnAddPoints && !lojaInativa) btnAddPoints.disabled = false;
            }
        }
        ...
        async function removeOnePoint() {
            ...
            try {
                ...
                // Não reabilita se a loja latchou inativa.
                if (btnRemovePoint && !lojaInativa) btnRemovePoint.disabled = false;
            }
        }
        ...
        if (btnDeliverPrize) {
            btnDeliverPrize?.addEventListener("click", async () => {
                ...
                try {
                    ...
                    const isInativa = isInativaErr(err);
                    if (isInativa) {
                        aplicarLojaInativa(); // latch: trava a UI no 1º erro
                        showAlert("error", "Loja com assinatura inativa", "A assinatura desta loja está inativa. Avise o responsável pelo estabelecimento.");
                    } else {
                        showAlert("error", "Erro ao entregar", "Não foi possível entregar o prêmio. Tente novamente.");
                    }
                    playSound("error");
                    // Só reabilita se NÃO latchou inativa.
                    if (!lojaInativa) btnDeliverPrize.disabled = false;
                }
            });
        }
